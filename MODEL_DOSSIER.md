...
| 2026-05-16 | Quoting: 5 changes — (1) truck charge toggle: checkbox "Show truck charge as separate line to customer", hidden → folded into SERVICE & DISPATCH FEE, persisted; (2-3) print table columns tightened (Lead Time 140→80, Unit Price 100→90, Amount 100→90, last-child CSS 120→90) giving Description more room; (4) parts Description changed from `<input>` to `<textarea>` for multiline Enter-key support, newlines render via `pre-wrap` + `<br>` on print/internal views; (5) tax toggle: "Include Sales Tax (5.5%) on quote" checkbox, unchecked → tax=0, tax line hidden from customer, internal view shows "N/A — included in pricing"; backward-compatible defaults for older quotes | HIGH | T4 | Opus | 88 | **`index.html`** only; no schema migration; older quotes load with showTruckToCustomer=false, showTax=true. |
