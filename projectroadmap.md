...
- **Recent Quotes Database — inline status dropdown.** `#recentQuotesDbTable` Status column renders a `.quote-history-status-select` cloned from `#quoteStatusInput` option list; **`updateQuoteStatusInline(quoteId, newStatus)`** writes `twinPillarsQuotesDB`, mirrors main-form rule (**`jobWorkflow` → `N/A`** when status ≠ **Approved**), syncs **`#quoteStatusInput`** if that row is the open quote, **`showSaveCue`**, **`renderQuoteHistory()`**. Wired once via **`wireRecentQuotesInlineStatus()`** on **`window.onload`**. Files: **`index.html`**.
